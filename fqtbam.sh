...
dir1="process"
dir2="data/bam/pbam"
files="data/raw/readname.mapping.txt" #This is that 5 column list that made above. its basically the i in (I think)

shortname=`head -n $SLURM_ARRAY_TASK_ID $files | tail -n 1 | cut -f1` #Shortname is column 1 of readname.mapping.txt the end of it is row column 
...
samtools merge -f $dir1/$shortname.temp.bam $dir1/$shortname.1.temp.bam $dir1/$shortname.2.temp.bam #merging bam files
